...
- **CI only; no functional changes.** The HACS validation workflow now runs without the
  `ignore: brands` exception: icons ship in-repo (`custom_components/zemismart_blinds/brand/`,
  served locally by Home Assistant 2026.3+), and the HACS action's brands check accepts that in
  place of a `home-assistant/brands` entry. This release exists so the HACS default-store
  submission links a release cut after the fully-unignored validation run, per the submission
  checklist.
